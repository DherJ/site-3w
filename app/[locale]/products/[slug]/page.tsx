...
import { getDefaultLead, getVariantForLead, isSizeValidForLead } from "@/components/products/hooks/getSelectedVariant";
...
export default function ProductPage({ params }: PageProps) {
  ...
  const galleryImages = product.images?.length ? product.images : [{ src: "/placeholder.jpg", alt: product.title }];

  // ✅ Variantes (state)
  const [selectedLead, setSelectedLead] = useState<LeadEquivalent>(() => getDefaultLead(product));
  ...
  const safeSize = useMemo(() => {
    return isSizeValidForLead(product, selectedLead, selectedSize) ? selectedSize : undefined;
  }, [product, selectedLead, selectedSize]);

  const variant = useMemo(() => getVariantForLead(product, selectedLead), [product, selectedLead]);
  ...
  // Specs / datasheet peuvent être overridées par variante si tu veux
        ...
        <section className="rounded-3xl bg-white/70 p-6 ring-1 ring-brandLine shadow-soft backdrop-blur md:p-10">
          <div className="grid gap-10 lg:grid-cols-[1.15fr_0.85fr] lg:items-start">
            ...
            <div>
              ...
              {/* Highlights (icônes + cards premium) */}
              ...
              {product.datasheetPdf ? (
                <div className="mt-8 rounded-3xl bg-white/60 p-6 ring-1 ring-brandLine">
                  ...
                  <div>
                    ...
                    {/* signature line en bas, à droite */}
